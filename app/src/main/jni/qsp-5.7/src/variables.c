...
unsigned char qspRand8[256] =
	{
		
	};
	...
	for (i = 0; i < src->IndsCount; ++i) {
	  ...
	  if (newInd >= 0 && newInd < maxCount) {
		if (count == dest->IndsBufSize) {
		  dest->IndsBufSize += 16;
		  dest->Indices =
			  (QSPVarIndex *)realloc(dest->Indices, dest->IndsBufSize * sizeof(QSPVarIndex));
		}
  ...
  while (ind <= count) {
	if (val->IsStr) {
	  ...
	  if (isRegExp) {
		...
		if (onig_match(onigExp, tempBeg, tempEnd, tempBeg, 0, ONIG_OPTION_NONE)
			== tempEnd - tempBeg) {
		  onig_free(onigExp);
		  return ind;
		}
