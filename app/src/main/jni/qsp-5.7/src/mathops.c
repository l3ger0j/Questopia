...
static void qspAddOperation(int opCode,
							int priority,
							QSP_FUNCTION func,
							int resType,
							int minArgs,
...
static int qspMathOpStringCompare(const void *name, const void *compareTo) {
  return qspStrsNComp((QSP_CHAR *)name,
					  ((QSPMathOpName *)compareTo)->Name,
...
static int qspFunctionOpCode(QSP_CHAR *funName) {
  ...
  name = (QSPMathOpName *)bsearch(uName,
...
static int qspOperatorOpCode(QSP_CHAR **expr) {
  ...
  for (i = 0; i < QSP_OPSLEVELS; ++i) {
	name = (QSPMathOpName *)bsearch(uStr,
									qspOpsNames[i],
									qspOpsNamesCounts[i],
									sizeof(QSPMathOpName),
									qspMathOpStringCompare);
	if (name) {
	  *expr += name->NameLen;
	  free(uStr);
	  return name->Code;
...
static QSPVariant qspValue(int itemsCount,
  ...
  while (1) {
	...
	if (!qspErrorNum) {
	  ...
	  switch (opCode) {
		case qspOpValue: tos = compValues[index];
		  break;
		case qspOpMul: QSP_NUM(tos) = QSP_NUM(args[0]) * QSP_NUM(args[1]);
		  break;
		case qspOpDiv:
		  if (!QSP_NUM(args[1])) {
		...
		case qspOpAdd:
		  ...
		  break;
		case qspOpSub: QSP_NUM(tos) = QSP_NUM(args[0]) - QSP_NUM(args[1]);
		  break;
		case qspOpMod:
		  if (!QSP_NUM(args[1])) {
			qspSetError(QSP_ERR_DIVBYZERO);
		  ...
		  QSP_NUM(tos) = QSP_NUM(args[0]) % QSP_NUM(args[1]);
		  break;
		case qspOpAppend: len = qspAddText(&QSP_STR(tos), QSP_STR(args[0]), 0, -1, QSP_TRUE);
		  QSP_STR(tos) = qspGetAddText(QSP_STR(tos), QSP_STR(args[1]), len, -1);
		  break;
		case qspOpEq: QSP_NUM(tos) = -(!qspAutoConvertCompare(args, args + 1));
		  break;
		case qspOpLt: QSP_NUM(tos) = -(qspAutoConvertCompare(args, args + 1) < 0);
		  break;
		case qspOpGt: QSP_NUM(tos) = -(qspAutoConvertCompare(args, args + 1) > 0);
		  break;
		case qspOpLeq: QSP_NUM(tos) = -(qspAutoConvertCompare(args, args + 1) <= 0);
		  break;
		case qspOpGeq: QSP_NUM(tos) = -(qspAutoConvertCompare(args, args + 1) >= 0);
		  break;
		case qspOpNe: QSP_NUM(tos) = -(qspAutoConvertCompare(args, args + 1) != 0);
		  break;
		case qspOpMinus: QSP_NUM(tos) = -QSP_NUM(args[0]);
		  break;
		case qspOpLoc: QSP_NUM(tos) = -(qspLocIndex(QSP_STR(args[0])) >= 0);
		  break;
		case qspOpObj: QSP_NUM(tos) = -(qspObjIndex(QSP_STR(args[0])) >= 0);
		  break;
		case qspOpNot: QSP_NUM(tos) = ~QSP_NUM(args[0]);
		  break;
		case qspOpAnd: QSP_NUM(tos) = QSP_NUM(args[0]) & QSP_NUM(args[1]);
		  break;
		case qspOpOr: QSP_NUM(tos) = QSP_NUM(args[0]) | QSP_NUM(args[1]);
		  break;
		  /* Embedded functions -------------------------------------------------------------- */
		case qspOpIIf: qspCopyVariant(&tos, QSP_NUM(args[0]) ? args + 1 : args + 2);
		  break;
		case qspOpLen: QSP_NUM(tos) = qspStrLen(QSP_STR(args[0]));
		  break;
		case qspOpIsNum: QSP_NUM(tos) = -qspIsCanConvertToNum(args);
		  break;
		case qspOpLCase: qspLowerStr(QSP_STR(tos) = qspGetNewText(QSP_STR(args[0]), -1));
		  break;
		case qspOpUCase: qspUpperStr(QSP_STR(tos) = qspGetNewText(QSP_STR(args[0]), -1));
		  break;
		case qspOpStr: QSP_STR(tos) = qspGetNewText(QSP_STR(args[0]), -1);
		  break;
		case qspOpVal:
		  ...
		  break;
		case qspOpArrSize: QSP_NUM(tos) = qspArraySize(QSP_STR(args[0]));
		  break;
		case qspOpTrim: QSP_STR(tos) = qspDelSpc(QSP_STR(args[0]));
		  break;
		case qspOpArrPos: QSP_NUM(tos) = qspArrayPos(args, argsCount, QSP_FALSE);
		  break;
		case qspOpArrComp: QSP_NUM(tos) = qspArrayPos(args, argsCount, QSP_TRUE);
		  break;
		case qspOpInput: QSP_STR(tos) = qspCallInputBox(QSP_STR(args[0]));
		  break;
		case qspOpRnd: QSP_NUM(tos) = qspRand() % 1000 + 1;
		  break;
		case qspOpCountObj: QSP_NUM(tos) = qspCurObjectsCount;
		  break;
		case qspOpMsecsCount: QSP_NUM(tos) = qspGetTime();
		  break;
		case qspOpQSPVer: QSP_STR(tos) = qspGetNewText(QSP_VER, QSP_LEN(QSP_VER));
		  break;
		case qspOpUserText: QSP_STR(tos) =
			  (qspCurInput ? qspGetNewText(qspCurInput, qspCurInputLen) :
				  qspGetNewText(QSP_FMT(""), 0));
		  break;
		case qspOpCurLoc: QSP_STR(tos) =
			  (qspCurLoc >= 0 ? qspGetNewText(qspLocs[qspCurLoc].Name, -1) :
				  qspGetNewText(QSP_FMT(""), 0));
		  break;
		case qspOpSelObj: QSP_STR(tos) =
			  (qspCurSelObject >= 0 ? qspGetNewText(qspCurObjects[qspCurSelObject].Desc, -1) :
				  qspGetNewText(QSP_FMT(""), 0));
		  break;
		case qspOpSelAct: QSP_STR(tos) =
			  (qspCurSelAction >= 0 ? qspGetNewText(qspCurActions[qspCurSelAction].Desc, -1) :
				  qspGetNewText(QSP_FMT(""), 0));
		  break;
		case qspOpMainText: QSP_STR(tos) = (qspCurDesc ? qspGetNewText(qspCurDesc, qspCurDescLen) :
			  qspGetNewText(QSP_FMT(""), 0));
		  break;
		case qspOpStatText: QSP_STR(tos) = (qspCurVars ? qspGetNewText(qspCurVars, qspCurVarsLen) :
			  qspGetNewText(QSP_FMT(""), 0));
		  break;
		case qspOpCurActs: QSP_STR(tos) = qspGetAllActionsAsCode();
		  break;
		  /* External functions -------------------------------------------------------------- */
		default: qspOps[opCode].Func(args, argsCount, &tos);
		  break;
	  }
	}
	if (argsCount) {
...
static void qspCompileExprPushOpCode(int *opStack,
									 int *opSp,
									 int *argStack,
									 int *argSp,
									 int opCode) {
...
static void qspAppendToCompiled(int opCode,
								int *itemsCount,
								QSPVariant *compValues,
								int *compOpCodes,
								int *compArgsCounts,
...
static int qspCompileExpression(QSP_CHAR *s,
								QSPVariant *compValues,
								int *compOpCodes,
								int *compArgsCounts) {
  ...
  int oldRefreshCount, opStack[QSP_STACKSIZE], argStack[QSP_STACKSIZE], opCode, itemsCount = 0,
	  opSp = -1, argSp = -1;
  ...
  while (1) {
	...
	if (waitForOperator) {
	  ...
	  while (qspOps[opCode].Priority <= qspOps[opStack[opSp]].Priority
		...
		qspAppendToCompiled(opStack[opSp],
							&itemsCount,
							compValues,
							compOpCodes,
							compArgsCounts,
	  ...
	  switch (opCode) {
		...
		case qspOpComma:
		  ...
		  break;
		default: qspCompileExprPushOpCode(opStack, &opSp, argStack, &argSp, opCode);
		  if (qspErrorNum) break;
		  waitForOperator = QSP_FALSE;
		  break;
	  }
	  ...
	  if (qspIsDigit(*s)) {
		...
		if (opCode >= qspOpFirst_Function) {
		  ...
		  free(name);
		  if (qspRefreshCount != oldRefreshCount || qspErrorNum) break;
		  qspAppendToCompiled(qspOpValue,
							  &itemsCount,
							  compValues,
							  compOpCodes,
							  compArgsCounts,
...
static void qspFunctionStrComp(QSPVariant *args, int count, QSPVariant *tos) {
  ...
  if (onig_new(&onigExp,
	...
	QSP_PNUM(tos) =
		-(onig_match(onigExp, tempBeg, tempEnd, tempBeg, 0, ONIG_OPTION_NONE) == tempEnd - tempBeg);
	onig_free(onigExp);
  }
}
