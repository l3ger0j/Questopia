...
static QSP_BOOL qspCheckQuest(char **strs, int count, QSP_BOOL isUCS2) {
  ...
  buf = (isOldFormat ?
	  qspGameToQSPString(strs[0], isUCS2, QSP_FALSE) :
	...
	if (isAddLoc) {
	  for (j = 0; j < actsCount; ++j) {
		qspLocs[locsCount].Actions[j].Image =
			(isOldFormat ? 0 : qspGameToQSPString(strs[ind++], isUCS2, QSP_TRUE));
...
static QSP_BOOL qspCheckGameStatus(QSP_CHAR **strs, int strsCount) {
  ...
  if (ind + 1 > strsCount) return QSP_FALSE;
  temp = qspReCodeGetIntVal(strs[ind++]);
  if (temp < 0 || temp > QSP_MAXOBJECTS || selObject >= temp
	  || (ind += 2 * temp) > strsCount)
	return QSP_FALSE;
  if (ind + 1 > strsCount) return QSP_FALSE;
  count = qspReCodeGetIntVal(strs[ind++]);
...
void qspOpenGameStatusFromString(QSP_CHAR *str) {
  ...
  qspFreeStrs(strs, count);
  qspIsMainDescChanged = qspIsVarsDescChanged = qspIsObjectsChanged = qspIsActionsChanged =
	  QSP_TRUE;
  qspOpenIncludes();
...
QSP_BOOL qspStatementOpenQst(QSPVariant *args, int count, QSP_CHAR **jumpTo, int extArg) {
  ...
  switch (extArg) {
	case 0: file = qspGetAbsFromRelPath(QSP_STR(args[0]));
