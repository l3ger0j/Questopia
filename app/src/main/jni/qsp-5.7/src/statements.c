...
static void qspAddStatement(int statCode,
							int extArg,
							QSP_STATEMENT func,
							int minArgs,
							int maxArgs,
...
static int qspGetStatCode(QSP_CHAR *s, QSP_CHAR **pos) {
  ...
  for (i = 0; i < QSP_STATSLEVELS; ++i) {
	name = (QSPStatName *)bsearch(uStr,
								  qspStatsNames[i],
								  qspStatsNamesCounts[i],
								  sizeof(QSPStatName),
								  qspStatStringCompare);
...
static int qspSearchElse(QSPLineOfCode *s, int start, int end) {
  ...
  while (start < end) {
	switch (s->Stats->Stat) {
	  case qspStatAct:
	  case qspStatIf: if (s->IsMultiline) ++c;
		break;
	  case qspStatElse:
	  case qspStatElseIf: if (c == 1) return start;
		break;
	  case qspStatEnd: if (!(--c)) return -1;
		break;
	}
	++start;
	++s;
...
static int qspSearchEnd(QSPLineOfCode *s, int start, int end) {
  ...
  while (start < end) {
	switch (s->Stats->Stat) {
	  ...
	  case qspStatIf: if (s->IsMultiline) ++c;
		break;
	  case qspStatEnd: if (!(--c)) return start;
		break;
	}
	++start;
	++s;
...
static QSP_BOOL qspExecString(QSPLineOfCode *s, int startStat, int endStat, QSP_CHAR **jumpTo) {
  ...
  for (i = startStat; i < endStat; ++i) {
	...
	switch (statCode) {
	  ...
	  case qspStatLabel:
	  case qspStatElse:
	  case qspStatEnd: break;
	  case qspStatComment:
	  case qspStatElseIf: return QSP_FALSE;
	  case qspStatAct: qspStatementAddAct(s, i, endStat);
		return QSP_FALSE;
	  case qspStatIf: return qspStatementIf(s, i, endStat, jumpTo);
	  case qspStatSet:
		if (i < s->StatsCount - 1) {
...
QSP_BOOL qspExecCode(QSPLineOfCode *s,
					 int startLine,
					 int endLine,
					 int codeOffset,
					 QSP_CHAR **jumpTo) {
...
void qspInitLineOfCode(QSPLineOfCode *line, QSP_CHAR *str, int lineNum) {
  ...
  if (*buf && statCode != qspStatComment) {
	...
	switch (statCode) {
	  case qspStatAct:
	  case qspStatIf:
	  case qspStatElseIf: delimPos = qspStrPos(buf, QSP_COLONDELIM, QSP_FALSE);
		if (delimPos) {
		  nextPos = delimPos + 1;
		  if (!(*nextPos)) isInLoop = QSP_FALSE;
		}
		break;
	  case qspStatElse: nextPos = qspSkipSpaces(paramPos);
		if (*nextPos == QSP_COLONDELIM[0]) ++nextPos;
		delimPos = (*nextPos ? nextPos : 0);
		break;
	  default: delimPos = qspStrPos(buf, QSP_STATDELIM, QSP_FALSE);
  ...
  switch (line->Stats->Stat) {
	...
	case qspStatElseIf:
	  line->IsMultiline =
		  (line->StatsCount == 1 && *(str + line->Stats->EndPos) == QSP_COLONDELIM[0]);
	  break;
	default: line->IsMultiline = QSP_FALSE;
	  break;
  }
  ...
  switch (extArg) {
	...
	case 2:
	  if (count)
		qspCurVarsLen = qspAddText(&qspCurVars, QSP_STR(args[0]), qspCurVarsLen, -1, QSP_FALSE);
	  qspCurVarsLen =
		  qspAddText(&qspCurVars, QSP_STRSDELIM, qspCurVarsLen, QSP_LEN(QSP_STRSDELIM), QSP_FALSE);
	  qspIsVarsDescChanged = QSP_TRUE;
	  break;
	case 3:
	  if (count)
		qspCurDescLen = qspAddText(&qspCurDesc, QSP_STR(args[0]), qspCurDescLen, -1, QSP_FALSE);
	  qspCurDescLen =
		  qspAddText(&qspCurDesc, QSP_STRSDELIM, qspCurDescLen, QSP_LEN(QSP_STRSDELIM), QSP_FALSE);
	  qspIsMainDescChanged = QSP_TRUE;
	  break;
	case 4:
	  qspCurVarsLen =
  ...
  switch (extArg) {
	...
	case 1:
	  if (qspClearText(&qspCurDesc, &qspCurDescLen))
		qspIsMainDescChanged = QSP_TRUE;
	  break;
	case 2: qspClearText(&qspCurInput, &qspCurInputLen);
	  qspCallSetInputStrText(0);
	  break;
	case 3: qspClearActions(QSP_FALSE);
	  break;
	case 4:
	  if (qspClearText(&qspCurVars, &qspCurVarsLen))
		qspIsVarsDescChanged = QSP_TRUE;
	  ...
	  qspCallSetInputStrText(0);
	  break;
	case 5: qspClearVars(QSP_FALSE);
	  qspClearObjectsWithNotify();
	  break;
	case 6: qspClearIncludes(QSP_FALSE);
...
static QSP_BOOL qspStatementShowWin(QSPVariant *args, int count, QSP_CHAR **jumpTo, int extArg) {
  ...
  switch (extArg) {
	case 0: qspCallShowWindow(QSP_WIN_ACTS, qspCurIsShowActs = val);
	  break;
	case 1: qspCallShowWindow(QSP_WIN_OBJS, qspCurIsShowObjs = val);
	  break;
	case 2: qspCallShowWindow(QSP_WIN_VARS, qspCurIsShowVars = val);
	  break;
	case 3: qspCallShowWindow(QSP_WIN_INPUT, qspCurIsShowInput = val);
	  break;
  }
  return QSP_FALSE;
}
